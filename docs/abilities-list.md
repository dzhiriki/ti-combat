...
- [x] **Linkship I / Linkship II** (Special Unit / Faction Technology) — Anti-Fighter Barrage 9 (x2), upgraded to Anti-Fighter Barrage 6 (x3). This unit can use the Space Cannon ability of one of your structures in its space area; Linkship I can trigger each structure once, while each Linkship II can trigger the same structure.
...
- [x] Il Na Viroset mech (Starlancer XI) — natively both a ship and a ground force. It participates on the active surface without needing another ship; newly placed mechs inherit both categories. Assign Hits Order and Sustain Priority use their standard controls. Anomaly bonus (+1 per anomaly in/adjacent) is a manual count input. Special combat-end rules are deferred.
