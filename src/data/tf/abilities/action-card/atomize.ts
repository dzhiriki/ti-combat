...
// model, every remaining unit on both sides. Modeled on Yin's Van Hauge
  ...
  invoke: [
    {
      ...
      call: ctx => {
        const opIds: UnitId[] = ctx.api.opponent.system
          .getUnitTypes()
          .flatMap(type =>
            ctx.api.opponent.system.getUnits(type, {
              includeVariants: true,
            }),
          )
        const ownIds: UnitId[] = ctx.api.own.system
          .getUnitTypes()
          .flatMap(type =>
            ctx.api.own.system.getUnits(type, { includeVariants: true }),
          )
